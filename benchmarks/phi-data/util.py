...
def agg_timestamp(response, events, startTime, endTime, externalServicesTime):
    ...
    priorServiceTime = startEvent['externalServicesTime'] + endEvent['externalServicesTime']
    #Remove double counting in case of overlap, assuming most of the external fns are just the ext. calls
    if startEvent['workflowEndTime'] > endEvent['workflowStartTime']:
        priorServiceTime -= startEvent['workflowEndTime'] - endEvent['workflowStartTime']
